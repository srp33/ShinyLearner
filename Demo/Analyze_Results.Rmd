...
In the current working directory, we should have directories named "Results_Basic" and "Results_ParamsOptimized", which contain the results of the classification analysis with and without hyperparameter optimization, respectively (see Execute_Algorithms.ipynb). These directories contains subdirectories labeled by dataset, each of which holds a "Metrics.tsv" file and a "Predictions.tsv" file. As an example, the relative path to the hyperparameter-optimized results for the iris dataset would be located at "Results_ParamsOptimized/iris/Metrics.tsv".

For later use, we will create paths to the Metrics.tsv files and store them in a vector.
...
Now we will extract data from the results files.
...
basic_metrics <- mutate(basic_metrics, Description = as.factor(Description)) %>%
  mutate(Algorithm = as.factor(Algorithm))
```
...
optimized_metrics <- mutate(optimized_metrics, Description = as.factor(Description)) %>%
  mutate(Algorithm = as.factor(Algorithm))
```
...
fs_metrics <- mutate(fs_metrics, Description = as.factor(Description)) %>%
  mutate(Algorithm = as.factor(Algorithm))
```
...
for (path in nested_fs_metrics_file_paths) {
  ...
  ranks = t(apply(tmp2, 1, function(x) { rank(x[(length(x) - 3):length(x)])}))
  tmp2[(ncol(tmp2) - 3):ncol(tmp2)] = ranks
...
nested_fs_metrics <- mutate(nested_fs_metrics,
                         Description = as.factor(Description),
...
make_bar_plot(plot_data,
              ...
              out_file_path="Graphs/Datasets_Basic_ElapsedTime.pdf")
```
...
make_faceted_point_plot(plot_data,
                        ...
                        out_file_path="Graphs/Datasets_Basic_AUROC.pdf")
```

## Relative performance of each algorithm (not hyperparameter optimized)
...
make_faceted_point_plot(plot_data2,
                        x="Description",
                        y="AUROC_Diff",
                        ylab="AUROC (difference from median)",
                        palette=colorPalette,
                        facet_column="Algorithm",
                        out_file_path="Graphs/Algorithms_Basic_AUROC.pdf",
                        y_line=0)
```
...
make_algo_comparison_plot(basic_metrics,
                          ...
                          out_file_path="Graphs/Algorithms_Compare_MLP.pdf")

make_algo_comparison_plot(basic_metrics,
                          algo1="sklearn/logistic_regression",
                          algo2="weka/SimpleLogistic",
                          out_file_path="Graphs/Algorithms_Compare_Logistic.pdf")
```

## Consistency of results across Monte Carlo iterations for each algorithm
...
make_bar_plot(plot_data,
              ...
              out_file_path="Graphs/Algorithms_SDev_AUROC.pdf")
```
...
ggsave("Graphs/Predictions_Histograms.pdf", width=dimension_1, height=dimension_2)
```
...
plot_data <- inner_join(basic_metrics, optimized_metrics, by=c("Description", "Iteration", "Algorithm")) %>%
  mutate(AUROC_Diff = Value.y - Value.x) %>%
...
make_faceted_point_plot(plot_data,
                        ...
                        palette=colorPalette,
                        facet_column="Algorithm",
                        out_file_path="Graphs/Algorithms_ParamsImprovement_AUROC.pdf",
                        y_line=0)
```

## Performance of different hyperparameter combinations

```{r}
#########################################
#TODO
#########################################
# default_results <- filter(nested_metrics, grepl("default__", Params)) %>%
#   rename(Default_AUROC=AUROC)
# 
# plot_data <- filter(nested_metrics, !grepl("default__", Params)) %>%
#   inner_join(default_results, by=c("Description")) %>%
#   rename(Params=Params.x) %>%
# #  select(-Params.y) %>%
#   mutate(AUROC_Diff = AUROC - Default_AUROC) %>%
#   mutate(AUROC_Diff = factor(round(AUROC_Diff, 1), levels=seq(-0.4, 0.4, 0.1))) %>%
# #  select(-AUROC, -Default_AUROC) %>%
#   mutate(Params=str_replace(Params, "alt__", "")) %>%
#   mutate(Params=str_replace_all(Params, "_", ", "))
# 
# ggplot(plot_data, aes(x = Params, y = Description)) +
#   geom_tile(aes(fill=AUROC_Diff), color="white") +
#   scale_fill_manual(name="Difference", values = c("#92c5de", "#f7f7f7", "#fddbc7", "#f4a582", "#b2182b")) +
#   xlab("Hyperparameters") +
#   ylab("Dataset") +
#   theme_bw() +
#   theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5))
# 
# ggsave("Graphs/Optimized_NestedMetrics_Heatmap.pdf", width=dimension_1, height=dimension_2)
```
...
make_faceted_point_plot(plot_data,
                        ...
                        palette=colorPalette,
                        facet_column="Algorithm",
                        out_file_path="Graphs/Algorithms_FSImprovement_AUROC.pdf",
                        y_line=0)
```

## Performance for combinations of classification and feature-selection algorithms
...
make_continuous_heatmap(plot_data,
                        ...
                        out_file_path="Graphs/FS_vs_CL.pdf")
```
...
make_continuous_heatmap(plot_data,
                        ...
                        legend_title=NULL,
                        out_file_path="Graphs/FS_NumFeatures.pdf")
```

## Top features selected

```{r}
#TODO: Can't do this one yet. When you can, make a heatmap with numbers in it.
#      Something like this: geom_text(aes(label = round(value, 1)))
```